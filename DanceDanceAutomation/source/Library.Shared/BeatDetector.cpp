...
const float BeatDetector::timeDilation = 10.0f;
...
void BeatDetector::LoadSong(int sSize, char* audioString)
{
	...
	channelMusic->setPitch(timeDilation);
...
void BeatDetector::updateTime()
{
	...
	currentTime = (currentTime - initialTime) * timeDilation;
	

	if (currentMinutes > 0)
		currentSeconds = (int)((currentTime / 1000) - (60 * currentMinutes));
	else
	...
	currentMinutes = (int)((currentTime / 1000) / 60);
...
float BeatDetector::calculateFluxAndSmoothing(float* currentSpectrum)
{
	...
	if (spectrumFluxes.size() > 0 && spectrumFluxes.size() < 10) //10
	{
		...
		//median = std::accumulate(spectrumFluxes.begin(), spectrumFluxes.end(), 0.0f);
		//median /= spectrumFluxes.size();

		if (smootherValues.size() > 0 && smootherValues.size() < 5) //5
		{
			...
			//smoothMedian = std::accumulate(smootherValues.begin(), smootherValues.end(), 0.0f);
			//smoothMedian /= smootherValues.size();
...
//is still playing
void BeatDetector::update(std::vector<TimeStamp>& beatTimings)
{
		...
		if (started)
		{
			...
			//Beat detected
			if (specFlux > beatThreshold && (clock() - timeBetween) > 350 / timeDilation) //350
			{
			...
			else if ((clock() - timeBetween) > 5000) //5000
			{
